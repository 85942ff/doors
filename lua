...
WindUI:Popup({
    Title = gradient("NOLSAKEN", Color3.fromHex("FFB6C1"), Color3.fromHex("FF69B4")),
    Icon = "sparkles",
    Content = "欢迎使用 NOLSAKEN",
    Buttons = {
        {
            Title = "取消",
...
function createUI()
    ...
    local Window = WindUI:CreateWindow({
        Title = "<font color='#FFB6C1'>N</font><font color='#FFA0B5'>O</font><font color='#FF8AA9'>L</font><font color='#FF749D'>S</font><font color='#FF5E91'>A</font><font color='#FF4885'>K</font><font color='#FF3279'>E</font><font color='#FF66B2'>N</font>",
    ...
    Window:Tag({ Title = "NOLSAKEN/doors", Color = Color3.fromHex("FF69B4") })
    ...
    Window:CreateTopbarButton("theme-switcher", "moon", function()
        ...
        WindUI:Notify({Title = "主题已切换", Content = "当前主题: " .. newTheme, Duration = 2})
    end, 990)
    ...
    local function createRainbowBorder(window, colorScheme, speed)
        ...
        glowEffect.Color = schemeData and schemeData[1] or COLOR_SCHEMES["樱花粉2"][1]
...
local SettingsTab = Window:Tab({Title = "设置", Icon = "settings"})
    ...
    local BorderSection = SettingsTab:Section({Title = "边框设置", Icon = "square"})
    BorderSection:Toggle({
        Title = "启用边框",
        Value = true,
        Callback = function(v)
            borderEnabled = v
            if rainbowStroke then rainbowStroke.Enabled = v end
            if v and not borderAnimation then
                borderAnimation = startBorderAnimation(Window, animationSpeed)
            elseif not v and borderAnimation then
    ...
    local colorNames = {}
    for name in pairs(COLOR_SCHEMES) do table.insert(colorNames, name) end
    BorderSection:Dropdown({
        ...
        Callback = function(v)
            currentColor = v
            if rainbowStroke then
                local glow = rainbowStroke:FindFirstChild("GlowEffect")
                if glow then
                    local schemeData = COLOR_SCHEMES[v]
                    if schemeData then glow.Color = schemeData[1] end
                end
            end
        end
    })
    ...
    Window:OnClose(function()
        if borderAnimation then borderAnimation:Disconnect() end
    end)
